...
class Reab3p16(nn.Module):
    ...
    def panel_comp_obj_pairs(self, objs, batch_size):
        ...
        obj_pairses_r = torch.zeros(batch_size, 2, 256 * 3, device = device)
        obj_pairses_c = torch.zeros(batch_size, 2, 256 * 3, device = device)
        obj_pairses = torch.zeros(batch_size, 54, 256 * 3, device = device)
    ...
    def ans_comp_obj_pairs(self, ans, pan, batch_size):
        ...
        obj_pairses_r = torch.zeros(batch_size, 1, 256 * 3, device = device)
        obj_pairses_c = torch.zeros(batch_size, 1, 256 * 3, device = device)
        obj_pairses = torch.zeros(batch_size, 26, 256 * 3, device = device)
    ...
    def forward(self, x):
        ...
        panel_embeddings = torch.zeros(batch_size, self.NUM_PANELS, 256, device = device)
        ...
        f_out = torch.zeros(batch_size, int(self.NUM_PANELS/2), device = device)

        # placeholder for type loss
        if self.type_loss:
            f_meta=torch.zeros(batch_size, 512, device = device)
